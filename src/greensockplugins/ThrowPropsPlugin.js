...
_gsScope._gsDefine(
  ...
  function(VelocityTracker) {
    var ThrowPropsPlugin = function(props, priority) {
      ...
      _calculateChange = (ThrowPropsPlugin.calculateChange = function(
        velocity,
        ease,
        duration,
        checkpoint
          ...
          if (
            ...
            if (typeof curProp !== 'object') {
              ...
              if (tracker && tracker.isTrackingProp(p)) {
                curProp =
                  typeof curProp === 'number'
                    ? { velocity: curProp }
                    : { velocity: tracker.getVelocity(p) } //if we're tracking this property, we should use the tracking velocity and then use the numeric value that was passed in as the min and max so that it tweens exactly there.
              } else {
                curVelocity = Number(curProp) || 0
                curDuration =
                  curVelocity * resistance > 0
                    ? curVelocity / resistance
                    : curVelocity / -resistance
              }
            }
            if (typeof curProp === 'object') {
      ...
      if (tween.vars.css) {
        tween.duration(
          _calculateTweenDuration(
            _cssProxy,
            { throwProps: _cssVars, ease: vars.ease },
            maxDuration,
            minDuration,
            overshootTolerance
          )
